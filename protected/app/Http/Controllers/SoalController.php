...
class SoalController extends Controller
{
  ...
  public function hapusdetailsoal()
  {
    if(Request::ajax()){
      ...
      File::Delete('assets/audios/'.$hapus->audio);
      $hapus->delete();
  ...
  public function hapus_audio()
  {
    if(Request::ajax()){
      ...
      $hapus = Detailsoal::where('id', $id_soal)->first();
      File::Delete('assets/audios/'.$hapus->audio);
      Detailsoal::where('id', $id_soal)->update(['audio' => '']);
    }
  }
  ...
  {
    if (Auth::user()->status == "G" or Auth::user()->status == "A") {
      $tampil = Input::get('tampil');
      $sesi = Input::get('sesi');

      $uniqname = date('ymdhis').rand(00000,99999);
      $savename = $uniqname.$_FILES["file"]["name"];

      $target_dir = "assets/audios/";
      $target_file = $target_dir . basename($savename);
      $uploadOk = 1;
      ...
      if($imageFileType != "wav" && $imageFileType != "wv" && $imageFileType != "m4a" && $imageFileType != "m4b" && $imageFileType != "m4p" && $imageFileType != "m4v" && $imageFileType != "m4r" && $imageFileType != "3gp" && $imageFileType != "mp4" && $imageFileType != "aac" && $imageFileType != "mp3" && $imageFileType != "WMA" ) {
        echo "Maaf hanya file dengan jenis WAV, WV, MP4, MP3, WMA dan ACC";
        $uploadOk = 0;
      }
      ...
      if ($uploadOk == 0) {
        ...
        if (move_uploaded_file($_FILES["file"]["tmp_name"], $target_file)) {
          echo "The file ". basename( $savename). " has been uploaded.";
        } else {
      ...
      if($tampil == 'N'){
        ...
        $query->audio = $savename;
        $query->sesi = $sesi;
        $query->save();
        return 'ok';
      }else{
        $query = Detailsoal::where('id', $tampil)->first();
        if($query->audio != ""){
          File::Delete('assets/audios/'.$query->audio);
        }
        $query->audio = $savename;
        $query->sesi = $sesi;
        $query->save();
